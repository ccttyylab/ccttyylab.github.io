...
- **输入框**:
  ...
  - `padding: 8px 12px`。
- **图标 (Icons)**:
  - **原则**: 优先使用 **Unicode 字符/文字符号** (如 `↑`, `↓`, `↻`, `⏻`) 替代多彩的 Emoji。
  - **目的**: 保持 UI 的简洁性、专业感和扁平化风格。
  - **例外**: 仅在没有找到合适的 Unicode 符号表达功能意图时，才考虑使用 Emoji。
  - **常用符号推荐**:
    - 向上/向下: `↑` / `↓`
    - 向左/向右: `←` / `→`
    - 刷新: `↻`
...
3.  **操作辅助**：必须在数据展示区域上方提供以下操作按钮：
    *   **导出数据**：点击后将当前核心配置生成 `.json` 文件并触发浏览器下载。文件名建议格式：`[脚本名]-config-[日期].json`。
    *   **导入数据**：点击后触发文件选择器 (`<input type="file">`)，读取用户选中的 `.json` 文件并覆盖当前配置（需包含数据校验和成功提示）。
    *   **还原默认数据**：点击后清除用户配置并恢复初始状态。
4.  **格式化要求**：
